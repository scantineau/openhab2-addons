    ...
    public void handleSiteUpdate(String command, int idPart) {
        if (HANDLED_COMMANDS.contains(command)) {
            try {
                ...
                logger.debug("handleSiteUpdate() : function = '{}': idPart = '{}'", command,
                        idPart, e);
            }
        } else {
                    ...
                    switch (channel.getUID().getId()) {
                        case CHANNEL_PART_ARM:
                            try {
                                ...
                                logger.warn("Got Exception during update state : CHANNEL_PART_ARM", e);
                            }
                            break;
                        case CHANNEL_PART_PARTIALLYARM:
                            try {
                                ...
                                logger.warn(
                                        "Got Exception during update state : CHANNEL_PART_PARTIALLYARM",
                                        e);
                            }
                            break;
                        case CHANNEL_PART_DISARM:
                            try {
                                ...
                                logger.warn("Got Exception during update state : CHANNEL_PART_DISARM",
                                        e);
                            }
                            break;
                    }
